...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    x_calc = x.reshape(x.shape[0], w.shape[0])
    out = x_calc.dot(w) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def softmax_loss(x, y):
    ...
    x_new = x.copy()
    x_new -= np.max(x, axis=1).reshape(-1,1)
    x_new = np.exp(x_new)
    x_new /= x_new.sum(axis=1).reshape(-1,1)
    #computing gradient
    dx = x_new.copy()
    dx[np.arange(len(y)),y] -= 1

    log_x = np.log(x_new + epsilon)
    
    loss = -np.sum(log_x[np.arange(len(y)),y])
    # log_scores(correct y class) : get the correct y class
    loss = loss / x.shape[0]
    dx = dx / x.shape[0]
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = x, x_normed, gamma, beta, mean, var, eps
...
def batchnorm_backward(dout, cache):
    ...
    dx = 0
    dxn = dout * gamma
    dvar =  (dxn * (x - mean) * (-1/2) * ((var + eps) ** (-3/2))).sum(axis=0)
    dmean = (dxn * (-1) / (np.sqrt(var + eps))).sum(axis=0) + dvar * (-2*(x - mean)).sum(axis=0)/N
    dx = dxn / (np.sqrt(var + eps)) + dvar * 2 * (x - mean) / N + dmean / N
...
def layernorm_backward(dout, cache):
    ...
    dgamma = (dout * cache[1].T).sum(axis=0)
    dbeta = dout.sum(axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
